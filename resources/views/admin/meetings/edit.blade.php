...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Edit Meeting</h5>
    </div>
    <div class="card-body">
        <form id="meetingForm">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="meeting_name" class="form-control" value="{{ $meeting->meeting_name }}" required>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="meeting_client" class="form-control select2">
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ $meeting->meeting_client == $client->id ? 'selected' : '' }}>
                                {{ $client->client_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="meeting_agenda" class="form-control" rows="4">{{ $meeting->meeting_agenda }}</textarea>
                </div>

                <div class="col-md-12 mb-3">
                    <label class="form-label">Meeting Minutes</label>
                    <textarea name="meeting_minutes" class="form-control" rows="4">{{ $meeting->meeting_minutes }}</textarea>
                </div>
            </div>
...
$('#meetingForm').submit(function(e) {
    ...
    $.ajax({
        url: '{{ route("admin.meetings.update", $meeting->id) }}',
        type: 'PUT',
        data: $(this).serialize(),
        ...
        error: function(xhr) {
            ...
            if(errors) {
                ...
                errorMsg = 'Failed to update meeting';
            }
